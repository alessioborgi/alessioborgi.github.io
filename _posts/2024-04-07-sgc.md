...
**Concrete example showing the collapse.** For a 3-layer GCN with weight matrices W₀, W₁, W₂ and ReLU activations:

```
GCN (3 layers):
  H¹ = ReLU( S̃ · X  · W₀ )     ← cannot pre-compute (W₀ is inside ReLU)
  H² = ReLU( S̃ · H¹ · W₁ )     ← depends on H¹
  H³ = ReLU( S̃ · H² · W₂ )     ← depends on H²

SGC (K=3): remove all intermediate nonlinearities, collapse all W:
  X̃ = S̃³ · X                    ← pre-compute ONCE (no parameters!)
  Ŷ = softmax( X̃ · W )          ← single linear layer trained on cached X̃
```

Pre-computation time: O(3·|E|·d) run once before training. Per-epoch cost: just O(N·d·C) — logistic regression speed.
