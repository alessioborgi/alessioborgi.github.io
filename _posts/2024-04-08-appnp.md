...
**Concrete worked example — 3 propagation steps, α=0.2:**

Suppose after the MLP transform, node v has H[v] = [1.0, 0.0] (its "prediction before propagation"). Its direct neighbour u has H[u] = [0.0, 1.0].

```
Z⁽⁰⁾ = H

Step 1:  Z⁽¹⁾ = (1−0.2)·S̃·Z⁽⁰⁾ + 0.2·H
         v receives:  0.8 × (avg of neighbours) + 0.2 × [1.0, 0.0]
         = 0.8·[0.0, 1.0] + 0.2·[1.0, 0.0] = [0.20, 0.80]

Step 2:  Z⁽²⁾ = 0.8·S̃·Z⁽¹⁾ + 0.2·H
         ≈ [0.36, 0.64]   (converging toward both, anchored to own features)

Step 3:  Z⁽³⁾ ≈ [0.49, 0.51]   (still distinguishable from neighbour's [0.51, 0.49])
```

Compare to plain 3-step GCN (no teleport): after 3 steps both nodes would approach the same average [0.5, 0.5] — oversmoothed. The teleport probability α=0.2 re-injects each node's original features at every step, preventing collapse.

<div style="background:#fff7ed;border-left:4px solid #f97316;border-radius:8px;padding:.95rem 1.1rem;margin:1.25rem 0;"><strong>Why This Works:</strong> The personalized PageRank closed form Z* = α(I − (1−α)S̃)⁻¹H is a weighted average over all walks starting from v — with longer walks getting exponentially decayed weight (factor (1−α)^k per step). Node v always stays most influenced by its own initial representation, regardless of how many steps are run.</div>
