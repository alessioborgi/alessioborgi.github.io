...
## Intuition First: Why Parameter Space is the Wrong Metric

Suppose your policy is a softmax over two actions, parameterised by a single logit $$\theta$$. A step of $$\Delta\theta = 0.01$$ when $$\theta = 0$$ changes action probabilities from 50%/50% to 50.25%/49.75% — a tiny behavioural change. The same step $$\Delta\theta = 0.01$$ when $$\theta = 10$$ changes probabilities from ~99.995%/0.005% to ~99.996%/0.004% — essentially nothing. The Euclidean distance in parameter space is completely uninformative about behavioural change. The Fisher information metric measures the *actual* divergence between policy distributions — making it the right ruler for policy optimisation.

<div style="background:#fff7ed;border-left:4px solid #f97316;border-radius:8px;padding:.95rem 1.1rem;margin:1.25rem 0;"><strong>Key Insight:</strong> The natural gradient is the direction of steepest ascent in the space of probability distributions (measured by KL divergence), not in parameter space. Two policies with very similar parameters can behave very differently, or two policies with very different parameters can be nearly identical behaviourally. The Fisher matrix corrects for this discrepancy.</div>
